...
public class Filtro<T extends Comparable<T>> { // adiciona restrição ao tipo genérico para que possa ser comparável
    ...
    public Filtro(List<T> listaOriginal, Comparator<T> comparador) {
        this.dadosOriginais = new ArrayList<>(listaOriginal); // cria uma cópia da lista original para evitar alterações
                                                              // inesperadas
    ...
    public List<T> filtrar(T elementoID) { // muda o tipo de retorno para List<T>, que é o tipo do objeto criado na
                                           // função
